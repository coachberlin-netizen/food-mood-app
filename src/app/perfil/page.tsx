...
import { 
  ...
  Settings, Download, Trash2, Globe, Heart, ExternalLink 
} from "lucide-react";
      ...
      <div className="min-h-screen bg-[var(--background)] pb-24">
        ...
        <header className="relative pt-32 pb-16 px-6 md:px-12 overflow-hidden">
          ...
          <div className="max-w-4xl mx-auto relative z-10 flex flex-col md:flex-row items-center md:items-end gap-8 text-center md:text-left">
            ...
            <div flex-1>
              <h1 className="text-4xl md:text-5xl font-serif text-navy mb-4 leading-[1.2]">{user?.name || "Viajero"}</h1>
              <p className="text-[11px] font-sans tracking-[0.2em] uppercase text-navy/50 flex flex-wrap items-center justify-center md:justify-start gap-4">
                <span>{user?.email || "hola@foodmood.app"}</span>
                <span className="hidden md:inline-block w-1 h-1 rounded-full bg-navy/20" />
            ...
            {predominantMood && (
              <div className="shrink-0 text-center">
                <p className="text-[10px] font-sans uppercase tracking-[0.2em] text-navy/40 mb-3">Tu estado más frecuente</p>
                <div 
                  className="inline-flex items-center gap-3 px-6 py-3 rounded-xl shadow-luxury border border-[#edeae3] bg-white"
        ...
        <div className="max-w-4xl mx-auto px-6 md:px-12 space-y-16">
          ...
          <section className="space-y-8">
            <div className="flex items-center gap-4">
              ...
              <h2 className="text-3xl font-serif text-navy">Tu Patrón</h2>
            </div>
            
            <div className="bg-white rounded-xl p-10 md:p-14 shadow-luxury border border-[#edeae3]">
              <div className="flex flex-col md:flex-row gap-8 items-center md:items-start">
                ...
                <div className="w-full md:w-1/3 space-y-3">
                  ...
                  <div className="flex flex-wrap gap-3 text-[11px] font-sans uppercase tracking-widest text-navy/60">
                ...
                {/* Insight */}
                <div className="flex-1 bg-[var(--background)] border border-[#edeae3] p-8 rounded-xl relative">
                   <p className="font-serif italic text-navy/80 text-xl leading-[1.6] shrink-0 max-w-lg">
                     &quot;{insight}&quot;
                   </p>
                </div>
              </div>
              ...
              <div className="mt-12 pt-8 border-t border-[#edeae3]">
                <p className="text-[10px] font-sans uppercase tracking-[0.2em] text-navy/40 mb-6">Últimos check-ins</p>
                <div className="flex gap-4 overflow-x-auto pb-4 scrollbar-hide">
                  {[...recentHistory].reverse().slice(0, 5).map((entry, idx) => {
                    const m = moods.find(x => x.id === entry.moodId);
                    return (
                      <div key={idx} className="flex-shrink-0 flex items-center gap-4 bg-[var(--background)] p-4 rounded-xl border border-[#edeae3] shadow-sm min-w-[200px] hover:border-navy/30 transition-colors cursor-default">
                        <div className="w-12 h-12 rounded-full flex items-center justify-center text-xl bg-white border border-[#edeae3]" style={{ color: m?.color }}>{m?.emoji}</div>
                        <div>
                          <p className="font-serif text-lg text-navy leading-none mb-1">{m?.nombre}</p>
                          <p className="text-[10px] font-sans uppercase tracking-[0.1em] text-navy/40">{new Date(entry.timestamp).toLocaleDateString()}</p>
                        </div>
                      </div>
                    )
                  })}
          ...
          <section className="space-y-8">
            <div className="flex items-center gap-4">
              <Bookmark className="w-5 h-5 text-navy/30" />
              <h2 className="text-3xl font-serif text-navy">Tus descubrimientos</h2>
            </div>
            
            <div className="flex gap-8 border-b border-[#edeae3] pb-4">
              <button 
                onClick={() => setActiveTab("guardadas")} 
                className={`text-[11px] font-sans uppercase tracking-[0.2em] transition-all relative pb-2 ${activeTab === 'guardadas' ? 'text-navy' : 'text-navy/40 hover:text-navy/60'}`}
              >
                Guardadas ({savedRecipes.length})
                {activeTab === 'guardadas' && (
                  <span className="absolute bottom-0 left-0 w-full h-[1px] bg-navy" />
                )}
              </button>
              <button 
                onClick={() => setActiveTab("completadas")} 
                className={`text-[11px] font-sans uppercase tracking-[0.2em] transition-all relative pb-2 ${activeTab === 'completadas' ? 'text-navy' : 'text-navy/40 hover:text-navy/60'}`}
              >
            ...
            {displayedRecipes.length > 0 ? (
              ...
              <div className="bg-white border border-dashed border-[#edeae3] p-16 rounded-xl text-center text-navy/40 font-light text-lg">
          ...
          <section className="space-y-8">
            <div className="flex items-center gap-4">
              <Activity className="w-5 h-5 text-navy/30" />
              <h2 className="text-3xl font-serif text-navy">Tu historia</h2>
            </div>
            
            <div className="bg-white rounded-xl p-10 md:p-14 shadow-luxury border border-[#edeae3] text-center md:text-left flex flex-col items-center md:items-start">
              {firstTest && latestTest && mFirst && mLatest ? (
                <p className="font-serif text-2xl md:text-3xl text-navy leading-[1.6] max-w-3xl">
                ...
                <p className="text-navy/50 font-light text-lg">Realiza pruebas para documentar tu historia emocional.</p>
              )}
            </div>
          </section>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            ...
            <section className="space-y-8">
              <div className="flex items-center gap-4">
                <Settings className="w-5 h-5 text-navy/30" />
                <h2 className="text-3xl font-serif text-navy">Ajustes</h2>
              </div>
              <div className="bg-white rounded-xl p-8 shadow-luxury border border-[#edeae3] space-y-2 divide-y divide-[#edeae3]">
                <div className="py-4 flex justify-between items-center cursor-pointer group">
                  <span className="font-light text-navy/80 group-hover:text-navy transition-colors">Idioma</span>
                  <span className="text-[10px] font-sans uppercase tracking-widest text-navy bg-[var(--background)] border border-[#edeae3] px-3 py-1.5 rounded-md">Español</span>
                </div>
                <div className="py-4 flex justify-between items-center cursor-pointer group">
                  <span className="font-light text-navy/80 group-hover:text-navy transition-colors">Notificaciones</span>
                  <div className="w-10 h-6 bg-green-400 rounded-full flex items-center p-1 justify-end"><div className="w-4 h-4 bg-white rounded-full" /></div>
                </div>
            ...
            <section className="space-y-8">
              <div className="flex items-center gap-4">
                <Heart className="w-5 h-5 text-navy/30" />
                <h2 className="text-3xl font-serif text-navy">Sobre Food·Mood</h2>
              </div>
              <div className="bg-white rounded-xl p-8 shadow-luxury border border-[#edeae3] flex flex-col justify-center gap-6 text-center md:text-left h-full pb-10">
                <h3 className="font-serif text-xl text-navy">Food·Mood v.1.0 (Beta)</h3>
                <div className="space-y-3">
                  <Link href="#" className="flex items-center justify-center md:justify-start gap-3 text-navy/60 hover:text-navy font-light transition-colors">
                ...
                <div className="pt-6 mt-4 border-t border-[#edeae3]">
                  <p className="text-[10px] text-navy/40 font-sans uppercase tracking-[0.2em]">Creado por</p>
                  <p className="text-sm font-light text-navy mt-2">Susana Ferreras — Psicología & Food Tech</p>
                </div>
              </div>
            </section>
          </div>
