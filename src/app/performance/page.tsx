  ...
  return (
    <div className="min-h-screen" style={{ background: '#F5F0E8' }}>
      ...
      <section className="pt-36 pb-20 px-10 text-center" style={{ background: 'linear-gradient(180deg, #4A1A26 0%, #6B2737 55%, #F5F0E8 100%)' }}>
        <p style={{ color: '#FF6B35', fontSize: 11, letterSpacing: '0.2em', textTransform: 'uppercase', marginBottom: 24 }}>Food Mood Performance</p>
          ...
          {estados.map((e) => (
            <button
              ...
              style={{
                ...
                border: selected === e.key ? '2px solid #FF6B35' : '2px solid transparent',
            ...
            >
              <div style={{ width: 52, height: 52, borderRadius: 14, background: selected === e.key ? 'rgba(245,240,232,0.12)' : 'rgba(107,39,55,0.08)', color: selected === e.key ? '#FF6B35' : '#6B2737', display: 'flex', alignItems: 'center', justifyContent: 'center', margin: '0 auto 14px' }}>
          ...
          <motion.section key={selected} initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 12 }} transition={{ duration: 0.4 }} style={{ maxWidth: 780, margin: '0 auto', padding: '0 40px 40px' }}>
            <div style={{ borderRadius: 20, overflow: 'hidden', background: '#FDFAF5' }}>
              <div style={{ background: '#6B2737', padding: '32px 36px 28px' }}>
                <p style={{ color: '#FF6B35', fontSize: 10, letterSpacing: '0.2em', textTransform: 'uppercase', marginBottom: 8 }}>{recipe.category}</p>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 32, padding: '32px 36px' }}>
                <div>
                  <h4 style={{ color: '#FF6B35', fontSize: 10, letterSpacing: '0.18em', textTransform: 'uppercase', marginBottom: 12 }}>Por que funciona</h4>
                  <p style={{ fontSize: 14, color: '#6B5560', lineHeight: 1.7 }}>{recipe.why}</p>
                </div>
                <div>
                  <h4 style={{ color: '#FF6B35', fontSize: 10, letterSpacing: '0.18em', textTransform: 'uppercase', marginBottom: 12 }}>Lo que necesitas</h4>
                  <ul style={{ listStyle: 'none', padding: 0, margin: 0 }}>
                    {recipe.ingredients.map((ing, i) => (
                      <li key={i} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '7px 0', borderBottom: i < recipe.ingredients.length - 1 ? '0.5px solid #EAE3D5' : 'none', fontSize: 14, color: '#2C1A20' }}>
                        <span style={{ width: 6, height: 6, borderRadius: '50%', background: '#FF6B35', flexShrink: 0 }} />{ing}
                      </li>
                    ))}
                  </ul>
                </div>
        ...
        {selected && (
          <motion.section initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.2 }} style={{ maxWidth: 780, margin: '0 auto', padding: '0 40px 80px' }}>
            <div style={{ background: '#4A1A26', borderRadius: 20, padding: 40, textAlign: 'center' }}>
              <p style={{ color: '#FF6B35', fontSize: 10, letterSpacing: '0.15em', textTransform: 'uppercase', marginBottom: 16 }}>40 recetas - 4 estados - Acceso inmediato</p>
              ...
              <Link href="/pricing" style={{ display: 'inline-block', background: '#FF6B35', color: '#4A1A26', padding: '16px 40px', borderRadius: 50, fontSize: 15, fontWeight: 500, textDecoration: 'none' }}>
