...
export async function generateMetadata({ params }: { params: { slug: string } }) {
  ...
  const { data } = await (await supabase).from('glossary').select('name, tagline').eq('slug', params.slug).single();
...
export default async function GlossaryDetailPage({ params }: { params: { slug: string } }) {
  ...
  const { data: profile } = user ? await (await supabase).from('profiles').select('premium_level').eq('id', user.id).single() : { data: null }
