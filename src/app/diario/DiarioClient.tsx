...
export default function DiarioClient({ initialIsPremium }: { initialIsPremium: boolean }) {
  ...
  const fetchData = useCallback(async (userId: string) => {
    ...
    const mAnalysis = analyzeMonth([analyzeWeek(monthlyEntries)]);
  ...
  useEffect(() => {
    ...
    init();
  }, [supabase, fetchData, initialIsPremium]);
            ...
            <section className="relative group">
              <div className="absolute inset-0 z-10 flex flex-col items-center justify-center p-6 text-center">
                 <div className="bg-white/80 backdrop-blur-md p-10 rounded-[2.5rem] shadow-2xl border border-[#6B2D3E]/10 max-w-lg flex flex-col items-center gap-6">
                    <div className="w-16 h-16 rounded-full bg-[#6B2D3E]/5 flex items-center justify-center text-[#6B2D3E]"><Lock className="w-8 h-8" /></div>
                    ...
                    <Link href="/pricing" className="bg-[#6B2D3E] text-white px-10 py-5 rounded-[60px] font-sans text-lg font-bold shadow-luxury hover:scale-105 transition-transform">Desbloquear con Premium → <span className="block text-xs opacity-60 font-normal">Desde 7€/mes</span></Link>
                 </div>
              </div>
              <div className="opacity-30 blur-md pointer-events-none select-none">
                <WeekMosaic colors={['#7EC8C8','#B8A9C9','#E8A87C','#7EC8C8','#F4E285','#B8A9C9','#E8A87C']} labels={DAY_LABELS} moods={['Calma', 'Reset', 'Activación', 'Calma', 'Focus', 'Reset', 'Activación']} hasNota={[true, false, true, true, false, true, false]} dominantMood="Calma" dominantColor="#7EC8C8" size="full" animate={false} />
              </div>
            </section>
          </div>
        ) : (
          <div className="flex flex-col gap-24">
            <section className="flex flex-col items-center"><QuickLog currentPalette={currentPalette} onSave={handleSaveQuickLog} /></section>
            {weekAnalysis && (
              <section className="flex flex-col gap-10">
                <div className="flex items-center gap-4"><Sparkles className="w-6 h-6 text-[#C9A84C]" /><h2 className="font-serif text-[28px] md:text-[32px] text-[#6B2D3E] font-bold italic">Esta semana</h2></div>
                <div className="bg-white p-10 rounded-[2.5rem] shadow-sm border border-[#6B2D3E]/5">
                    <WeekMosaic colors={weekAnalysis.colorSequence} labels={DAY_LABELS} moods={weekAnalysis.entries.map(e => e.mood_dominante)} hasNota={weekAnalysis.entries.map(e => e.nota !== null)} dominantMood={weekAnalysis.dominantMood} dominantColor={weekAnalysis.dominantColor} size="full" animate={true} />
                    <div className="mt-12 flex flex-col gap-4 text-center max-w-2xl mx-auto">{weekAnalysis.pattern && <p className="font-sans text-[18px] text-[#6B2D3E] italic">&quot;{weekAnalysis.pattern}&quot;</p>}<p className="font-sans text-[16px] text-[#6B7280] leading-relaxed">{weekAnalysis.recommendation}</p></div>
                </div>
              </section>
            )}
            {monthAnalysis && (
              <section className="flex flex-col gap-10">
                <div className="flex items-center gap-4"><BookOpen className="w-6 h-6 text-[#C9A84C]" /><h2 className="font-serif text-[28px] md:text-[32px] text-[#6B2D3E] font-bold italic">Este mes</h2></div>
                <MonthMosaic colorGrid={monthAnalysis.colorGrid} moodGrid={monthAnalysis.moodGrid} monthName={monthAnalysis.monthName} moodDistribution={monthAnalysis.moodDistribution} insight={monthAnalysis.insight} animate={true} />
              </section>
            )}
            {historyMonths.length > 0 && (
              <section className="flex flex-col gap-10">
                <div className="flex items-center gap-4"><Calendar className="w-6 h-6 text-[#C9A84C]" /><h2 className="font-serif text-[28px] md:text-[32px] text-[#6B2D3E] font-bold italic">Tu historia emocional</h2></div>
                    ...
                    {historyMonths.map((m) => {
                        ...
                        return (
                            <div key={m.id} className="bg-white rounded-3xl overflow-hidden border border-[#6B2D3E]/5 shadow-sm">
                                ...
                                <AnimatePresence>{isOpen && (<motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} className="border-t border-[#6B2D3E]/5 p-10 flex justify-center bg-gray-50/50"><MonthMosaic colorGrid={m.analysis.colorGrid} moodGrid={m.analysis.moodGrid} monthName={m.name} moodDistribution={m.analysis.moodDistribution} insight={m.analysis.insight} animate={false} /></motion.div>)}</AnimatePresence>
                            </div>
                        );
                    })}
                </div>
