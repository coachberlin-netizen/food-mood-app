...
import { VALUES_MI_SYSTEM_PROMPT } from "@/lib/behavioral/prompts"
...
const BodySchema = z.object({
  messages: z.array(MessageSchema).max(MAX_TURNS * 2 + 1),
})
...
export async function POST(req: NextRequest) {
  ...
  const systemFull = [
    VALUES_MI_SYSTEM_PROMPT,
    isLastTurn
      ? "\n\nEste es el TURNO FINAL. Cierra con el formato exacto: \"Valores identificados: X, Y, Z. Acción comprometida: [acción].\""
  ...
  try {
    ...
    logger.error({ err }, "behavioral/values: error IA")
