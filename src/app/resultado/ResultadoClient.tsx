...
export default function ResultadoClient({ initialIsPremium, initialUser, moodParam }: ResultadoClientProps) {
  const storeMood = useQuizStore((s) => s.resultMood);
  const moodIdSource = moodParam || storeMood || "activacion";
  
  // Use the passed props for source of truth
  ...
  return (
    <div className="min-h-screen bg-[var(--background)]" style={{ background: `linear-gradient(to bottom, ${moodStyle.color}10, transparent)` }}>
      <div className="max-w-3xl mx-auto px-6 py-12 md:py-20 md:px-12">
        ...
        {/* ── Enhanced Upsell (ONLY rendered for non-premium) ─────────────────────────────── */}
        {!isPremium && (
          <motion.div
